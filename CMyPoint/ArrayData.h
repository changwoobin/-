...
public:
	...
	template <typename E>
	friend ostream& operator<<(ostream& out, const ArrayData<E>& arr);
...
template <typename T>
ArrayData<T>::ArrayData(ArrayData<T>& arr) :ArrayData(arr.capacity, arr.used)
{
	cout << "복사생성자" << endl;
...
template <typename T>
ArrayData<T>::ArrayData(ArrayData<T>&& arr) noexcept :capacity(arr.capacity), used(arr.used), data(move(arr.data)) {
	cout << "이동생성자" << endl;
	//arr.data = nullptr;
}
